...
namespace BitSits_Framework
{
    ...
    class GameplayScreen : GameScreen
    {
        ...
        private void DrawScore(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            //spriteBatch.DrawString(gameContent.debugFont, "fps : " + fps.ToString("00"), Vector2.Zero, Color.White);

            //spriteBatch.DrawString(gameContent.debugFont, "X = " + (int)mousePos.X + " Y = " + (int)mousePos.Y,
            //    new Vector2(0, 20), Color.White);

            score = Math.Min(score + (float)gameTime.ElapsedGameTime.TotalSeconds * 50, prevScore + level.Score);

            spriteBatch.DrawString(gameContent.gameFont,
                "level: " + levelIndex + " / " + MaxLevelIndex, new Vector2(320, 0), Color.White, 0, 
                Vector2.Zero, 0.75f, SpriteEffects.None, 1);
        }
